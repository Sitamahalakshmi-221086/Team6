  /* STATE */
    ...
    /* ✅ No keys in HTML — all handled by server.js */
    async function sendOtpEmail(toEmail, toName, otp) {
      const res = await fetch('http://localhost:5005/send-email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: toEmail, name: toName || 'Recruiter', otp: otp })
      });
      if (!res.ok) {
    ...
    async function goNext(from) {
      ...
      try {
        await sendOtpEmail(email, contact, generatedOtp);
        isDevMode = false;
        ...
        showToast('✅ OTP sent to ' + email, '#059669');
      } catch (err) {
        ...
        showToast('⚠️ Server not reachable – using dev mode', '#f59e0b', 4000);
      }
    ...
    function goTo(stage) {
      const curId = currentStage === 2 ? 'stage-2-company' : 'stage-' + currentStage;
      const el = document.getElementById(curId);
      if (el) el.classList.remove('active');
      currentStage = stage;
      const newId = stage === 2 ? 'stage-2-company' : 'stage-' + stage;
    ...
    function runCheck(c, fail) {
      const el = document.getElementById(c.id); const errEl = document.getElementById(c.err);
      if (!el || !errEl) return;
    ...
    function checkPwd(input, prefix, labelId) {
      ...
      lbl.textContent = texts[score] || ''; lbl.style.color = colors[score];
    }
    ...
    function addRole(e) {
      ...
      hiringRoles.push(val); renderRoles(); input.value = '';
    ...
    function verifyOtp() {
      ...
      errEl.style.display = 'none'; errEl.style.color = '#ef4444';
      if (otp.length < 6) { errEl.textContent = 'Please enter all 6 digits.'; errEl.style.display = 'block'; return; }
      if (Date.now() > otpExpiry) {
        errEl.textContent = '⏰ OTP has expired. Please resend.'; errEl.style.display = 'block';
      ...
      setTimeout(() => {
        ...
        if (otp === generatedOtp) {
          clearInterval(resendTimer); showToast('🎉 Email verified!', '#059669'); goTo(4);
        } else {
          errEl.textContent = '❌ Incorrect OTP. Please check and try again.'; errEl.style.display = 'block';
          otpInputs.forEach(i => { i.value = ''; i.style.borderColor = '#ef4444'; });
          otpInputs[0].focus();
          setTimeout(() => otpInputs.forEach(i => i.style.borderColor = ''), 1200);
        }
    ...
    function startResendTimer(seconds = 30) {
      ...
      resendTimer = setInterval(() => {
        t--; document.getElementById('timer-count').textContent = t;
        if (t <= 0) {
          clearInterval(resendTimer);
    ...
    async function resendOtp() {
      ...
      otpInputs.forEach(i => { i.value = ''; i.style.borderColor = ''; });
      document.getElementById('otp-error').style.display = 'none';
      generatedOtp = generateOtp(); otpExpiry = Date.now() + OTP_TTL_MS;
      try {
        await sendOtpEmail(email, contact, generatedOtp);
        isDevMode = false;
        document.getElementById('dev-otp-display').style.display  = 'none';
        document.getElementById('demo-mode-banner').style.display = 'none';
        document.getElementById('otp-sent-badge').style.display   = 'inline-block';
        showToast('✅ New OTP sent to ' + email, '#059669');
      } catch (err) {
        isDevMode = true;
        document.getElementById('dev-otp-value').textContent      = generatedOtp;
        document.getElementById('dev-otp-display').style.display  = 'block';
      ...
      btn.disabled = false; startResendTimer(); otpInputs[0].focus();
    }

    function showToast(msg, bg = '#059669', duration = 2500) {
