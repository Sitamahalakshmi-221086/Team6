...
async function goNext(from, event) {
  ...
  if (from === 2) {
    ...
    try {
      const response = await fetch('http://localhost:5001/api/companies/signup', {
        method: 'POST',
...
function addRole(e) {
  ...
  input.value = '';
  const rolesErr = document.getElementById('err-c-roles');
  if (rolesErr) rolesErr.classList.remove('show');
}
...
function renderRoles() {
  ...
  hiringRoles.forEach(s => {
    ...
    tag.innerHTML = s + `<button type="button" onclick="removeRoleTag('${s}')">×</button>`;
